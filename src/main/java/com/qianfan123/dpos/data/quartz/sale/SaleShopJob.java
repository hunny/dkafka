...
public class SaleShopJob implements Job {
  ...
  private final Logger logger = LoggerFactory.getLogger(SaleShopJob.class);
  ...
  public void execute(JobExecutionContext arg0) throws JobExecutionException {
    ...
    shopService.handle(new BatchHandler() {
      ...
      public void handle(List<String> shops, String... strings) {
        ...
        logger.info("处理数据库[{}]的所有门店信息。", dbName);
        for (String shop : shops) {
          JobDataMap JobDataMap = new JobDataMap();
          JobDataMap.put(ShopService.DB_NAME, dbName);
          JobDataMap.put(ShopService.SHOP_ID, shop);
          try {
            jobService.startNow(shop, dbName, JobDataMap, SaleUuidJob.class, false);
